...
from vortex.flows.streaming.strategies import eventhub_kafka_to_delta
...
@patch("vortex.flows.streaming.strategies.eh_kafka_read_stream", new_callable=MagicMock)
...
def test_eventhub_kafka_to_delta(
    ...
    result = eventhub_kafka_to_delta(job_params, spark)
...
@patch("vortex.flows.streaming.strategies.eh_kafka_read_stream", new_callable=MagicMock)
def test_eventhub_kafka_to_delta_with_read_stream_error(mock_eh_read_stream):
...
@patch("vortex.flows.streaming.strategies.transformation", new_callable=MagicMock)
@patch("vortex.flows.streaming.strategies.delta_write_stream", new_callable=MagicMock)
def test_eventhub_kafka_to_delta_with_empty_data(
    mock_delta_write_stream, mock_transformation, mock_eh_read_stream
):
