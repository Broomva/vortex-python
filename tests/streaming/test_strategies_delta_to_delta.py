...
from vortex.flows.streaming.strategies import delta_to_delta
...
def test_delta_to_delta(
    ...
    result = delta_to_delta(job_params, spark)
...
@patch("vortex.flows.streaming.strategies.delta_read_stream", new_callable=MagicMock)
def test_delta_to_delta_with_read_stream_error(mock_eh_read_stream):
...
@patch("vortex.flows.streaming.strategies.transformation", new_callable=MagicMock)
@patch("vortex.flows.streaming.strategies.delta_write_stream", new_callable=MagicMock)
def test_delta_to_delta_with_empty_data(
    mock_delta_write_stream, mock_transformation, mock_eh_read_stream
):
